...
export function StatementClient({ borrower, loansWithSummary }: Props) {
  ...
  const statementText = `ACCOUNT STATEMENT - ${statementDate}
...
SUMMARY
━━━━━━━━━━━━━━━━━━━━━━━
Outstanding Principal: ${formatCurrency(grandTotals.principal)}
Total Interest Received: ${formatCurrency(grandTotals.received)}
Pending Interest: ${formatCurrency(grandTotals.pending)}
Total Outstanding: ${formatCurrency(grandTotals.principal + grandTotals.pending)}

${loansWithSummary.map(({ loan, summary }) => `
LOAN: ${loan.loanNumber}
Principal: ${formatCurrency(Number(loan.principalAmount))}
Current Principal: ${formatCurrency(Number(loan.currentPrincipal))}
Rate: ${Number(loan.interestRate)}% / ${loan.loanFrequency === "MONTHLY" ? "month" : "day"}
Started: ${formatDate(loan.startDate)}
Status: ${loan.status}
Interest Received: ${formatCurrency(summary.totalInterestReceived)}
Pending Interest: ${formatCurrency(summary.pendingInterest + summary.overdueInterest)}
`).join("\n")}

Statement generated: ${statementDate}
Generated by GuruFinance - Smart Loan & Interest Management.`;

  const waLink = createWhatsAppLink(borrower.mobile, statementText);
